 ...
 */
test('login, add to cart and proceed to checkout', { tag: '@auto-exr' }, async ({ session }) => {
  ...
  await automationExercise.navigateToFullUrl(config.get('automationExerciseBaseUrl') as string)
