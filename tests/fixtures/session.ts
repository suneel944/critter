...
export const test = base.extend<FX>({
  session: async ({}, use) => {
    ...
    async function getOrInitProvider(
      name: ProviderName,
    ): Promise<IDeviceProvider> {
    ...
    const session: Session = {
      ...
      apiSend: async (clientOrConfig, request) => {
        const client =
          clientOrConfig instanceof ApiClient
            ? clientOrConfig
      ...
      mobile: async (caps: Caps, options) => {
        const providerName: ProviderName =
          options?.provider ||
          (process.env.DEVICE_PROVIDER as ProviderName) ||
          "local"
