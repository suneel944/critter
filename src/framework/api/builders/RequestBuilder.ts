...
export default class RequestBuilder<M extends HttpMethod = 'GET'> {
  ...
  // NOTE: with exactOptionalPropertyTypes, use explicit unions instead of `?`
  private dataBody: unknown
  private jsonBody: unknown
  private formBody: Record<string, string> | undefined
  private multipartBody: MultipartType | undefined
  ...
  method_<T extends HttpMethod>(m: T): RequestBuilder<T> {
    ;(this as unknown as RequestBuilder<T>).method = m
