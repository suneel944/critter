...
export default class RequestBuilder<M extends HttpMethod = "GET"> {
  ...
  static get(path: string)     { return new RequestBuilder<"GET">().method_("GET").path_(path) }
  /** Create a POST request builder. */
  static post(path: string)    { return new RequestBuilder<"POST">().method_("POST").path_(path) }
  /** Create a PUT request builder. */
  static put(path: string)     { return new RequestBuilder<"PUT">().method_("PUT").path_(path) }
  /** Create a DELETE request builder. */
  static delete(path: string)  { return new RequestBuilder<"DELETE">().method_("DELETE").path_(path) }
  /** Create a PATCH request builder. */
  static patch(path: string)   { return new RequestBuilder<"PATCH">().method_("PATCH").path_(path) }
  /** Create a HEAD request builder. */
  static head(path: string)    { return new RequestBuilder<"HEAD">().method_("HEAD").path_(path) }
  /** Create an OPTIONS request builder. */
  static options(path: string) { return new RequestBuilder<"OPTIONS">().method_("OPTIONS").path_(path) }
  ...
  path_(p: string) { this.path = p; return this }

  /** Reuse a Playwright APIRequestContext (keeps cookies/headers). */
  context(ctx: APIRequestContext) { this.ctx = ctx; return this }

  /** Add a single query parameter. */
  query(name: string, value: string | number | boolean) { this.params[name] = value; return this }

  /** Merge multiple query parameters. */
  queries(obj: Record<string, string | number | boolean> = {}) { Object.assign(this.params, obj); return this }

  /** Add a single header (case-insensitive). */
  header(name: string, value: string) { this.headers.add(name, value); return this }

  /** Merge multiple headers. */
  headers_(obj: Record<string, string> = {}) { this.headers.addAll(obj); return this }
  ...
  data(v: unknown) { this.dataBody = v; this.jsonBody = undefined; this.formBody = undefined; this.multipartBody = undefined; return this }

  /** Attach a JSON body (auto-serialized). */
  json(v: unknown) { this.jsonBody = v; this.dataBody = undefined; this.formBody = undefined; this.multipartBody = undefined; return this }

  /** Attach a URL-encoded form body. */
  form(v: Record<string, string>) { this.formBody = v; this.dataBody = undefined; this.jsonBody = undefined; this.multipartBody = undefined; return this }

  /** Attach a multipart form body. */
  multipart(v: MultipartType) { this.multipartBody = v; this.dataBody = undefined; this.jsonBody = undefined; this.formBody = undefined; return this }

  /** Merge additional Playwright request options. */
  options(opts: Partial<RequestOptions>) { Object.assign(this.extra, opts); return this }
  ...
  public async build(): Promise<BuiltRequest<M>> {
    ...
    const ctxForAuth: AuthContext = { ...this.authCtx, path: this.path, method: this.method }
    ...
    const options: RequestOptions = {
      ...
      ...(bodyAllowed && this.dataBody !== undefined ? { data: this.dataBody } : {}),
      ...(bodyAllowed && this.jsonBody !== undefined ? { json: this.jsonBody } : {}),
      ...(bodyAllowed && this.formBody ? { form: this.formBody } : {}),
      ...(bodyAllowed && this.multipartBody ? { multipart: this.multipartBody } : {}),
