 ...
 * - Can bind to an existing browser, context, or page session.
 */
export class PlaywrightAdapter implements IAdapter {
   ...
   * - `"waitForSelector"` — `{ selector, state?, timeoutMs? }` → boolean
  ...
  async execute(action: string, params?: unknown): Promise<unknown> {
    ...
    switch (action) {
      ...
      case 'waitForSelector': {
        ...
        // With exactOptionalPropertyTypes, omit keys rather than pass undefined.
      ...
      case 'screenshot': {
        ...
        const opts: PageScreenshotOptions = { fullPage: !!fullPage }
  ...
  /** Tear down the session (page → context → browser). */
