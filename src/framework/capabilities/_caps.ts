import { CapabilityBuilder, type Caps } from './CapabilityBuilder'
...
function pickPlatformName(caps?: Record<string, unknown>): string {
  ...
  const pn1 = typeof caps.platformName === 'string' ? caps.platformName : undefined
  const pn2Raw = caps['appium:platformName']
  const pn2 = typeof pn2Raw === 'string' ? pn2Raw : undefined
  return (pn1 ?? pn2 ?? '').trim().toLowerCase()
}

export function detectPlatformFromCaps(caps?: Record<string, unknown>): Platform {
  const pn = pickPlatformName(caps)
  return pn.includes('ios') ? 'ios' : 'android'
}

export function toBuilder(input: MobileCapsInput, platform: Platform): CapabilityBuilder {
  const base = platform === 'ios' ? CapabilityBuilder.ios() : CapabilityBuilder.android()
...
export function buildCaps(builder: CapabilityBuilder, mutable = true): Record<string, unknown> {
  return mutable ? builder.buildMutable() : (builder.build() as Record<string, unknown>)
}
